ages = [12,18,33,84,45,67,12,82,95,16,10,23,43,29,40,34,30,16,44,69,70,74,38,65,36,83,50,11,7,9,64,78,37,3,8,68,22,4,60,33,82,45,23,5,18,28,99,17,81,14,88,50,19,59,7,44,93,35,72,25,63,11,69,11,76,10,60,30,14,21,82,47,6,21,88,46,78,92,48,36,28,51]

# print(ages)
# ages.sort()
# print(ages)

# Part 3 Time Calculator
# String """ """ can be used as multiple lines as one string
print("""
Time Calculator
1- Add 2 times 
2- Find the difference between 2 times 
3- Convert to Hours 
4- Convert to Minutes 
5- Convert Minutes to Time 
6- Convert Hours to Time 
7- Convert Days to Time 
8- Exit 
""")

option = int(input("Enter an option: "))

if option == 1:
    # Do our add function
    time1 = input("Please enter time in format HH:MM:SS: ")
    time2 = input("Please enter time in format HH:MM:SS: ")
    time1Array = time1.split(":")
    time2Array = time2.split(":")
    print(time1Array)
    print(time2Array)

    seconds = int(time1Array[2]) + int(time2Array[2])
    minutes = int(time1Array[1]) + int(time2Array[1])
    hours = int(time1Array[0]) + int(time2Array[0])

    while seconds > 60:
        seconds -= 60
        minutes += 1

    while minutes > 60:
        minutes -= 60
        hours += 1

    print(f"{hours}:{minutes}:{seconds}")